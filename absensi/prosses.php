		...
		if ($_POST['jenis_login'] == "guru") {
			...
			if(mysqli_num_rows($queryLoginUser) == 1){
				while ($user = mysqli_fetch_array($queryLoginUser)) {
					...
					if($_SESSION['akses'] == 'operator' || $_SESSION['akses'] == 'admin'){
						...
						$checkWaliKelas = mysqli_query($konek,"SELECT * FROM ruangan WHERE guru =".$user['id_guru']);
						if (mysqli_num_rows($checkWaliKelas) >0) {
							# code...
							$_SESSION['akses']='wali kelas';
			...
			if(mysqli_num_rows($queryLoginUser) == 1){
				while ($user = mysqli_fetch_array($queryLoginUser)) {
					...
					$_SESSION['username'] = $user['username'];
					$_SESSION['password'] = $user['password'];
					$_SESSION['ruangan']  = $user['ruangan_siswa'];
